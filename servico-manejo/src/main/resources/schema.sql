...
-- DLQ (Dead Letter Queue) do servico-manejo -- registros PERMANENTES do que
-- deixou de ser processado (poison message, erro de deserializacao ou de
-- regra de negocio nos fluxos de pesagem e embarque). Pela regra do ADR-002,
-- o sistema nao corrige o passado: o que falhou fica registrado aqui para
-- auditoria/reprocessamento manual, e nunca e' apagado por um consumidor.
-- Append-only: nenhuma linha e' alterada nem deletada pelo fluxo normal.
--
-- A UNIQUE (origem_topico, particao, deslocamento) e' a deduplicacao: se o
-- processo morrer entre a gravacao na DLQ e o commit do offset do topico
-- original, a mesma mensagem pode ser entregue de novo e gravada de novo --
-- essa restricao aceita a primeira gravacao e descarta o resto, o mesmo
-- padrao de evento_processado, mas ancorado na POSICAO Kafka da mensagem
-- (que existe mesmo para poison message sem ce_id aproveitavel).
CREATE TABLE IF NOT EXISTS evento_dlq (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    origem_topico VARCHAR(255) NOT NULL,
    particao INTEGER NOT NULL,
    deslocamento BIGINT NOT NULL,
    chave VARCHAR(255),
    evento_id VARCHAR(64),
    tipo_evento VARCHAR(128),
    payload VARCHAR(4000) NOT NULL,
    motivo VARCHAR(255) NOT NULL,
    detalhe VARCHAR(4000),
    registrado_em TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (origem_topico, particao, deslocamento)
);

CREATE INDEX IF NOT EXISTS idx_evento_dlq_pesquisa
    ON evento_dlq (origem_topico, registrado_em);
